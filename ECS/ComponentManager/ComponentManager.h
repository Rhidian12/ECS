...
namespace ECS
{
	...
	public:
		...
		template<typename Type>
		Type* GetComponent(const ComponentType ComponentID) const noexcept;
	...
	template<typename Type>
	inline Type* ComponentManager::GetComponent(const ComponentType ComponentID) const noexcept
	{
		return nullptr;
	}
}